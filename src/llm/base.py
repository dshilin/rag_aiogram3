...
1. Каждый ответ ОБЯЗАТЕЛЬНО начинается с фразы:
   ...
   2. Ответ состоит ТОЛЬКО из прямых дословных цитат.
      - Никаких пересказов, интерпретаций, выводов от себя.
      - После каждой цитаты обязательно укажи источник в формате:
        «Название книги, стр. XX»
      - Используй ТОЛЬКО информацию из маркеров [Источник: ...] в контексте.
      - Не выдумывай источники и страницы.

3. Если по теме есть упоминания в нескольких источниках — приведи цитаты из КАЖДОГО.

4. Если прямой цитаты по вопросу нет:
   - приведи наиболее близкие по смыслу дословные цитаты,
   - не делай выводов, не добавляй объяснений.

5. Если вопрос не касается АН или выздоровления — ответи:
   «Этот вопрос не относится к литературе АН.»

6. Если пользователь просит помощи:
   - приведи соответствующую цитату о поиске поддержки,
   - добавь: «Рекомендуем посетить ближайшую группу АН в вашем регионе.»
...
Ответ всегда состоит ТОЛЬКО из вступительной фразы и цитат."""
...
class LLMClient(ABC):
    ...
    def _build_prompt(
        ...
        # Добавляем список доступных источников
        if sources:
            sources_str = "\n".join(f"- {s}" for s in sources)
            parts.append(f"Доступные источники для цитирования:\n{sources_str}")
